...
export default function AdminProductDetail() {
  ...
  const stockColor = outOfStock ? 'text-error-text' : lowStock ? 'text-warning' : 'text-success-text';
  const discount =
      ...
      {product && (
        <main className="mx-auto max-w-[1440px] px-5 pb-24 pt-9 sm:px-10">
          <nav aria-label="Breadcrumb" className="flex items-center gap-2 text-[13px] text-dim">
            <Link to="/admin" className="text-muted transition-colors hover:text-primary-lighter">
              Admin
            </Link>
            {product.category && (
              <>
          ...
          <div className="mt-7 grid grid-cols-1 items-start gap-8 lg:grid-cols-[1.15fr_1fr] lg:gap-14">
            <div>
              ...
              <div className="mt-3.5 grid grid-cols-3 gap-3.5">
                {[1, 2, 3].map((i) => (
                  <div key={i} className="aspect-[4/3] overflow-hidden rounded-xl border border-line bg-surface">
                    <ProductImage
            ...
            <div>
              ...
              {product.colors && product.colors.length > 0 && (
                <div className="mt-7">
                  <div className="mb-2.5 text-[13px] font-semibold text-[#CBD5E1]">
                    Color
                    {selectedColor && <span className="font-normal text-muted"> — {selectedColor.name}</span>}
                  </div>
                    ...
                    {product.colors.map((color) => (
                      <button
                        ...
                        className={`h-[34px] w-[34px] rounded-full ring-offset-2 ring-offset-background transition-shadow ${color.class} ${
                          selectedColor?.name === color.name ? 'ring-2 ring-primary-hover' : 'ring-1 ring-line'
                        }`}
                      />
                    ))}
                  </div>
              ...
              {outOfStock ? (
                <button disabled className="mt-[30px] w-full cursor-not-allowed rounded-btn border border-line bg-surface px-6 py-[15px] text-[15px] font-semibold text-dim">
                  Out of stock
                </button>
              ) : (
                <button onClick={handleCart} type="button" className="mt-[30px] w-full rounded-btn bg-primary px-6 py-[15px] text-[15px] font-semibold text-white transition-colors hover:bg-primary-hover">
                  Add to cart
                </button>
              )}
              ...
              <Link
                ...
                className="mt-3 flex w-full items-center justify-center rounded-btn border border-line bg-surface-raised px-6 py-[13px] text-[14px] font-semibold text-[#CBD5E1] transition-colors hover:border-[#475569] hover:text-content"
              >
                Edit product
              </Link>

              <div className="mt-8 border-t border-line-subtle pt-[26px]">
                <div className="text-[13px] font-semibold uppercase tracking-[0.06em] text-dim">Description</div>
                <p className="mt-3 text-[14.5px] leading-[1.7] text-muted">{product.description}</p>
              </div>

              {product.highlights && product.highlights.filter(Boolean).length > 0 && (
                <div className="mt-6">
                  <div className="text-[13px] font-semibold uppercase tracking-[0.06em] text-dim">Highlights</div>
                  <div className="mt-3 flex flex-col gap-2.5">
                    {product.highlights.filter(Boolean).map((highlight) => (
                      <div key={highlight} className="flex gap-2.5 text-sm leading-[1.5] text-[#CBD5E1]">
